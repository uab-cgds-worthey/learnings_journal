...
A script with all recommended commands based on commonly used pipelines,
directories, and tools.

#### **How to use**

Locate `.bashrc` and `.zshrc` in the `$HOME` directory and add the following
lines:
...
| guber_logs                             | Change directory to logs for gubernomics pipeline.                                                                                  |
| guberstat n                            | Get status of gubernomics pipeline runs active within the last 'n' days                                                             |
| snakestat \<logfile path\>             | Get status of all queued SLURM jobs for a snakemake pipeline (parsed from supplied stderr log file)                                 |
